...
function processScript(obj) {
    ...
    if (_transpile) {
        ...
        let bundle = browserify({
            ...
            .bundle()
        bundle
            .pipe(vinyl(_concat ? _concat : 'main.js'))
            .pipe(buffer())
            .pipe(_hash ? hash(fileNameFormat) : noop())
    ...
    var imageCompress = src(CONF.TINYPNG_API_KEY ? obj.files.concat('!**/*.{png,jpg,jpeg}') : obj.files, { cwd: obj.dir, allowEmpty: true })
        .pipe(
            image({
                ...
                guetzli: ['--quality', 85],
                gifsicle: ['--optimize'],
                svgo: ['--disable', 'cleanupIDs', '--disable', 'convertColors', '--disable', 'removeViewBox'],
                quiet: !CONF.VERBOSE
            })
        )
        .pipe(obj.hash ? hash(fileNameFormat) : noop())
...
function processFont(obj) {
    ...
    fonts.pipe(
        through.obj(function(file, enc, cb) {
            ...
            if (obj.formats.indexOf(ext) < 0) del.sync(path, { force: true })
            cb(null)
        })
    )
    return fonts
    ...
    CONF.FONT_SOURCE_FILES.forEach(function(obj, index) {
        ...
        fontFiles.forEach(function(path, index) {
            ...
            var fontFamily = fullFontName
                .split(' ')
                .filter(item => !props.includes(item))
                .join(' ')
...
function configUpdated(done) {
    ...
    series(clear, processFonts, processImages, processStyles, processScripts, processPages, processAnotherFiles)(done)
}
...
task('dev', done => {
    ...
    series(clear, processFonts, processImages, processStyles, processScripts, processPages, processAnotherFiles, processBrowserSync)(done)
})
